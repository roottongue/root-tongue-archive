                        ...
                        <?php $thumb = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), '' );
							...
							if(get_field('audio_url')!=''){
								...
								<div class="grid <?php echo $type; ?>" data-video-url="<?php echo $video; ?>"
                                    style="background-color:rgba(208, 220, 40, 0.9);">
                                    <p><?php the_title();?></p>
							...
							<?php } else {
                                ...
                                $thumb_url = $video.".jpeg";
                                $handle = curl_init($thumb_url);
                                curl_setopt($handle,  CURLOPT_RETURNTRANSFER, TRUE);
                                $response = curl_exec($handle);
                                $httpCode = curl_getinfo($handle, CURLINFO_HTTP_CODE);

                                if($httpCode >= 200 && $httpCode <= 400) {
                                    $url_to_use = $thumb_url;
                                } else {
                                    $url_to_use = $url;
                                }
                                curl_close($handle);
								...
								?>
								<div class="grid <?php echo $type; ?>" data-video-url="<?php echo $video; ?>"
                                    style="background-image:url(<?php echo $url_to_use; ?>);">
